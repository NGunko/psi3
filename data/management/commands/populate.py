...
deserunt mollit anim id est laborum."""[init:end]
	...
	def addCategory(self, noCategories):
		...
		for num in range (noCategories):
			cat=Category()
			cat.name="category %d"%(num+1)
			cat.created=datetime.datetime.now(tz=timezone.utc)
			cat.tooltip="tooltip %d" %(num+1)
			cat.save()
		print Category.objects.all()
	...
	def addWorkflow(self, noWorkflows):
		...
		# add apropriate code
		# create fake json
		for num in range (noWorkflows):
			work=Workflow()
			work.name="workflow %d" %(num+1)
			work.created=datetime.datetime.now(tz=timezone.utc)
			work.views=0
			work.downloads=0
			work.versionInit="versionInit %d" %(num+1)
			work.client_ip= "%d.%d.%d.%d" %(random.randint(0,255),random.randint(0,255),random.randint(0,255),random.randint(0,255))
			work.keywords="keywords %d" %(num+1)
			work.json=self.getJson()
			work.save()
			cat=Category.objects.get(name="category %d" %random.randint(1,5))
	...
	},
	{
		...
		for c in Category.objects.all():
			for p in Workflow.objects.filter(category=c):
